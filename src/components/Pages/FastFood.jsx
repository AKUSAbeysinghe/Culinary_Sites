...
const FastFoodHero = () => {
  return (
    <div
      className="relative w-full h-screen bg-cover bg-center"
      style={{ backgroundImage: `url(${FastFoodBanner})` }}
    >
      <div className="relative z-10 flex flex-col items-center justify-center h-full px-6 md:px-24 text-white text-center bg-black bg-opacity-30 rounded-lg p-6">
        <h1 className="text-4xl md:text-6xl font-bold leading-tight text-shadow">
          Fast Food <br />
...
const BurgersAndSandwiches = ({ items }) => {
  return (
    <div className="bg-white py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {items.length === 0 ? (
          <p className="text-center text-gray-600 col-span-full">No items available</p>
        ) : (
          items.map((item) => (
            <div key={item.id} className="bg-white rounded-lg shadow-lg overflow-hidden">
              ...
              <div className="p-6">
                <h3 className="text-xl font-semibold text-gray-800">{item.name}</h3>
                <p className="mt-2 text-gray-600 text-sm">{item.description || "N/A"}</p>
                <p className="mt-4 text-xs text-gray-500">{item.chef || "N/A"}</p>
                <button className="mt-6 w-full py-3 bg-red-500 text-white font-semibold rounded-full hover:bg-red-600 transition duration-300">
                  Add to Cart
                </button>
              </div>
...
const App = () => {
  ...
  // Fetch menu items for Fast Food category (category_id: 1)
  ...
  const burgersAndSandwiches = menuItems.filter((item) => item.sub_category_id === SUBCATEGORY_IDS.BURGERS_SANDWICHES);
  const friedChicken = menuItems.filter((item) => item.sub_category_id === SUBCATEGORY_IDS.FRIED_CHICKEN);
  const sides = menuItems.filter((item) => item.sub_category_id === SUBCATEGORY_IDS.SIDES);

  return (
    <div>
      {error && (
        <div className="p-4 mb-6 rounded-lg bg-red-200 text-red-800 border-2 border-red-400 font-medium text-center">
          {error}
        </div>
      )}
      <FastFoodHero />
      <BurgersAndSandwiches items={burgersAndSandwiches} />
      <FriedChicken items={friedChicken} />
      <Sides items={sides} />
      <FastFoodServices />
      <CallToAction />
    </div>
