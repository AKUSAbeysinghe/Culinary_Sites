  ...
  const menuCategories = [
    {
      ...
      category: "Starters",
      description: "Kick off your meal with our flavorful selection of appetizers, soups, and salads",
      dishes: 7, // Adjust based on actual data
      priceRange: "$5.99 - $8.99",
    },
  ];
  ...
  const fetchMenuItems = async () => {
    ...
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        // Filter for Starters (category_id: 2)
        const starterItems = data.data.filter(item => item.category_id === "2" || item.category_id === 2);
        setAppetizerItems(starterItems.filter(item => item.sub_category_id === "4" || item.sub_category_id === 4));
        setSoupItems(starterItems.filter(item => item.sub_category_id === "5" || item.sub_category_id === 5));
        setSaladItems(starterItems.filter(item => item.sub_category_id === "6" || item.sub_category_id === 6));
      } else {
    ...
    <div>
      ...
      {isLoading && (
        <div className="text-center py-12">
          <p className="text-gray-600 font-poppins">Loading starters...</p>
        </div>
      )}
